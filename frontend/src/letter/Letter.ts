export class Letter {
    ...
    constructor(private _letter: string, private _linkedLetters: Letter[]) {
        if (_letter.length !== 1) {
    ...
    increase(amount: number) {
        ...
        this._letter = String.fromCharCode(this.getResultingCharCode(currentCharCode + amount));
        for (const linkedLetter of this._linkedLetters) {
            linkedLetter.increase(amount);
        }
    }
